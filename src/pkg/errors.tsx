...
export function handleGetFlowError<S>(
  ...
  function isFlowErrorResponse(data: any): data is FlowErrorResponse {
    return (
      data &&
      typeof data === 'object' &&
      ('error' in data || 'redirect_browser_to' in data)
    )
  }

  return async (err: AxiosError) => {
    if (!err.response || !isFlowErrorResponse(err.response.data)) {
      return Promise.reject(err) // or handle this case appropriately
    }
