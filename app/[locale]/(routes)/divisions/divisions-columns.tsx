...
export type DivisionColumnsEvents = {
  ...
  handlerOpenSheet: (divisionData: DivisionType, isEditable: boolean) => void
...
export const getDivisionsColumns = (divisionsEvents: DivisionColumnsEvents) => {
      ...
      cell: ({ row }: ColumnRow) => (
        <div className="flex pl-3">
          <Pencil
            ...
            onClick={() => divisionsEvents.handlerOpenSheet(row.original, true)}
          />
        </div>
      )
    },
    ...
    
    {
      ...
      cell: ({ row }: ColumnRow) => (
        <div className="flex pl-4">
          <MoreHorizontal
            className="h-4 w-4 cursor-pointer"
            onClick={() => divisionsEvents.handlerOpenSheet(row.original, false)}
          />
        </div>
      )
    }
